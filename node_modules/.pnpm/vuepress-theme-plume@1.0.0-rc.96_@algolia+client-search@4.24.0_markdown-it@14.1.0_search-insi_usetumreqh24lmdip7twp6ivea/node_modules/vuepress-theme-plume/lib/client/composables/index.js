...
var blogPostData = ref4(blogPostDataRaw);
function usePostList() {
  return blogPostData;
}
function useLocalePostList() {
  const locale = usePageLang2();
  return computed2(() => blogPostData.value.filter((item) => item.lang === locale.value));
}
if (__VUEPRESS_DEV__ && (import.meta.webpackHot || import.meta.hot)) {
...
import { onUnmounted, readonly, ref as ref5, watch } from "vue";
import { inBrowser as inBrowser2 } from "../utils/index.js";
var focusedElement = ref5();
var active = false;
var listeners = 0;
function useFlyout(options) {
  const focus = ref5(false);
  if (inBrowser2) {
    if (!active) {
...
import { computed as computed5, ref as ref6, watch as watch2 } from "vue";
import { useRoute } from "vuepress/client";
import { normalizeLink as normalizeLink2, resolveNavLink } from "../utils/index.js";
function useNavbarData() {
  const { theme } = useData();
  return computed5(() => resolveNavbar(theme.value.navbar || []));
}
function resolveNavbar(navbar, _prefix = "") {
  const resolved = [];
  navbar.forEach((item) => {
    if (typeof item === "string") {
      resolved.push(resolveNavLink(normalizeLink2(_prefix, item)));
    } else {
      const { items: items2, prefix, ...args } = item;
      const res = { ...args };
      if ("link" in res) {
        res.link = normalizeLink2(_prefix, res.link);
      }
      if (items2?.length) {
        res.items = resolveNavbar(
          items2,
          normalizeLink2(_prefix, prefix)
        );
      }
      resolved.push(res);
    }
...
import { resolveRouteFullPath, useRoute as useRoute3, useRouteLocale as useRouteLocale2 } from "vuepress/client";
import {
  ensureLeadingSlash,
  isArray,
  isPlainObject,
...
import { isActive, normalizeLink as normalizeLink3, normalizePrefix, resolveNavLink as resolveNavLink2 } from "../utils/index.js";

// src/client/composables/encrypt.ts
import { compare, genSaltSync } from "bcrypt-ts/browser";
import { computed as computed6, inject as inject3, provide } from "vue";
import { hasOwn, useSessionStorage } from "@vueuse/core";
import { useRoute as useRoute2 } from "vuepress/client";

// src/client/composables/encrypt-data.ts
import { encrypt as rawEncrypt } from "@internal/encrypt";
import { ref as ref7 } from "vue";
var encrypt = ref7(resolveEncryptData(rawEncrypt));
...
function setupEncrypt() {
  ...
  const hasPageEncrypt = computed6(() => {
...
var sidebarData = ref8(items);
var autoDirSidebar = ref8(__auto__);
if (__VUEPRESS_DEV__ && (import.meta.webpackHot || import.meta.hot)) {
...
function resolveSidebarItems(sidebarItems, _prefix = "") {
  ...
  sidebarItems.forEach((item) => {
    if (isString(item)) {
      ...
      const navLink = { ...args };
      if (link) {
        navLink.link = normalizeLink3(_prefix, link);
        const nav = resolveNavLink2(navLink.link);
...
function useSidebar() {
  ...
  const routeLocal = useRouteLocale2();
  const is960 = useMediaQuery("(min-width: 960px)");
  const { isPageDecrypted } = useEncrypt();
  const isOpen = ref8(false);
  const sidebarKey = computed7(() => {
  ...
  const leftAside = computed7(() => {
    if (hasAside.value) {
      return frontmatter.value.aside == null ? theme.value.aside === "left" : frontmatter.value.aside === "left";
    }
    return false;
  });
  const isSidebarEnabled = computed7(() => hasSidebar.value && is960.value);
  const sidebarGroups = computed7(() => {
...
function useSidebarControl(item) {
  ...
  const collapsed = ref8(false);
  const collapsible = computed7(() => {
    return item.value.collapsed != null;
  });
  const isLink = computed7(() => {
    return !!item.value.link;
  });
  const isActiveLink = ref8(false);
  ...
  watch3([() => page.value.path, item, () => route.hash], updateIsActiveLink);
  onMounted(updateIsActiveLink);
  const hasActiveLink2 = computed7(() => {
    if (isActiveLink.value) {
      return true;
    }
    return item.value.items ? containsActiveLink(page.value.path, item.value.items) : false;
  });
  const hasChildren = computed7(() => {
    return !!(item.value.items && item.value.items.length);
  });
  watchEffect(() => {
    collapsed.value = !!(collapsible.value && item.value.collapsed);
  });
  watch3(() => [page.value.path, isActiveLink.value, hasActiveLink2.value], () => {
...
function useAside() {
  ...
  const isAsideEnabled = computed8(() => {
...
import { inject as inject5, onMounted as onMounted2, onUnmounted as onUnmounted3, onUpdated, provide as provide3, ref as ref9 } from "vue";
...
import { resolveRouteFullPath as resolveRouteFullPath2, usePageLang as usePageLang3, useRoute as useRoute4 } from "vuepress/client";
...
function usePrevNext() {
  ...
  const postList = usePostList();
  const locale = usePageLang3();
  const { isBlogPost } = useBlogPageData();
  const prevNavList = computed9(() => {
...
function resolveFromSidebarItems(sidebarItems, currentPath, offset) {
  const index = sidebarItems.findIndex((item) => resolveRouteFullPath2(item.link) === currentPath);
  if (index !== -1) {
...
import { usePageLang as usePageLang4 } from "vuepress/client";
import { computed as computed11, onMounted as onMounted3, ref as ref10, watchEffect as watchEffect2 } from "vue";
function useLastUpdated() {
  const { theme, page, frontmatter } = useData();
  const lang = usePageLang4();
  const date = computed11(() => page.value.git?.updatedTime ? new Date(page.value.git.updatedTime) : null);
  const isoDatetime = computed11(() => date.value?.toISOString());
  const datetime = ref10("");
  const lastUpdatedText = computed11(() => {
...
import { computed as computed13, onMounted as onMounted4, onUnmounted as onUnmounted4 } from "vue";
var lightTint = {
...
function useInternalLink() {
  ...
  const blogData = computed14(() => theme.value.blog || {});
  const home = computed14(() => resolveLink("home", "/"));
  const blog = computed14(() => blogData.value.postList !== false ? resolveLink("blog", blogData.value.link || "blog/") : home.value);
  const tags = computed14(() => blogData.value.tags !== false ? resolveLink("tag", blogData.value.tagsLink || "blog/tags/") : void 0);
  const archive = computed14(() => blogData.value.archives !== false ? resolveLink("archive", blogData.value.archivesLink || "blog/archives/") : void 0);
  const categories = computed14(() => blogData.value.categories !== false ? resolveLink("category", blogData.value.categoriesLink || "blog/categories/") : void 0);
  return {
    home,
    blog,
    tags,
...
import { customRef, nextTick as nextTick2, toValue, watch as watch4 } from "vue";
import { useRoute as useRoute5, useRouter } from "vuepress/client";
...
function usePostListControl(homePage) {
  ...
  const blog = computed15(() => theme.value.blog || {});
  const is960 = useMediaQuery3("(max-width: 960px)");
  const postList = computed15(() => {
  ...
  const isLastPage = computed15(() => page.value >= totalPage.value);
  const isFirstPage = computed15(() => page.value <= 1);
  const isPaginationEnabled = computed15(() => blog.value.pagination !== false && totalPage.value > 1);
  const finalList = computed15(() => {
    if (blog.value.pagination === false)
...
var tagColorsRef = ref11(articleTagColors);
...
function useTags() {
  ...
  const tags = computed16(() => {
    const tagMap = {};
    list.value.forEach((item) => {
      if (item.tags) {
        toArray2(item.tags).forEach((tag) => {
          if (tagMap[tag])
            tagMap[tag] += 1;
          else
            tagMap[tag] = 1;
  ...
  const postList = computed16(() => {
    if (!currentTag.value)
      return [];
    return list.value.filter((item) => {
      if (item.tags)
        return toArray2(item.tags).includes(currentTag.value);
      return false;
    }).map((item) => ({
      title: item.title,
      path: item.path,
...
import { resolveRouteFullPath as resolveRouteFullPath3, useRoute as useRoute6 } from "vuepress/client";
...
function useLink(href, target) {
  ...
  const link = computed20(() => {
    ...
    return resolveRouteFullPath3(link2, currentPath);
  });
